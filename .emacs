...
(if (not (file-exists-p "~/.emacs.d/elpa/undo-tree-0.8.2/undo-tree.elc"))
    (if (file-exists-p "~/.emacs.d/elpa/undo-tree-0.8.2/undo-tree.el")
        (progn
          (byte-compile-file "~/.emacs.d/elpa/undo-tree-0.8.2/undo-tree.el" t)
...
;; NOTE(earl): I used this one
;; (require 'highlight-indent-guides)

;; NOTE(earl): This one
;; To enable it automatically in most programming modes, use the following:
...
;; fill:      The default method. All whitespace used for indentation is highlighted.
;;            The color of each level of indentation alternates between highlight-indent-guides-odd-face and
;;            highlight-indent-guides-even-face.
;; column:    Like fill, but only the first column of each level of indentation is highlighted.
;; character: The first column of each level of indentation is drawn using a column of characters.
;;            The character to draw with is specified by highlight-indent-guides-character,
;;            and it is drawn using the face highlight-indent-guides-character-face.

;; NOTE(earl): And this one
;; (setq highlight-indent-guides-method 'character)

;; To change the character used for drawing guide lines with the character display method,
;; customize highlight-indent-guides-character.
...
;; By default, this mode dynamically chooses colors that look acceptable with the loaded theme.
;; It does this by altering the luminosity of the theme's background color by a given percentage.
;; These percentages can be tweaked, to make the colors more intense or subtle.
...
;; Responsive guides allow you to visualize not only the indentation itself,
;; but your place in it. To enable this feature, customize highlight-indent-guides-responsive,
...
;;        This changes as the cursor moves.
;; stack: Like top, but also use a third color for all "ancestor" guides of the current guide.

;; Again, this will change as the cursor moves around.

;; By default, responsive guides are not updated immediately every time the cursor moves.
;; Instead, guides only update after the cursor stops moving for a certain period of time
;; (one tenth of a second, by default). If you would like to change this behavior,
;; customize highlight-indent-guides-delay, and set it to the number of seconds to wait.
;; For example, to disable the delay entirely:
;; (setq highlight-indent-guides-delay 0)

;; Enabling this feature provides more highlight faces,
;; as well as more color modifiers for the dynamic colors feature.
...
;; The highlighter function is the function that calculates which faces to use to display each guide character.
;; If the default highlighter function isn't doing it for you, you can write your own by customizing
;; highlight-indent-guides-highlighter-function. A custom highlighter takes three parameters:
...
;; display:    The display method setting. One of fill, column, or character.

;; A custom highlighter should return the face to use to color the given guide character.
;; Alternatively, it may return nil to specify that the guide should not be displayed at all.

;; The highlighter function is called once for each indentation character,
;; each time a section of the buffer is re-highlighted. To speed things up a little,
;; the results of the highlighter function are memoized per-character, and are reused when possible.
;; Because of this, a custom highlighter should run quickly, and should not have side-effects
;; (i.e. it should not depend on or change external values that might differ from one call to the next).
;; A custom highlighter can return custom faces, but those faces will not be recognized by the dynamic color feature,
;; and will need to be defined and colored manually.

;; The following example highlighter will highlight normally,
;; except that it will not highlight the first two levels of indentation:
...
;; To display the character method guides, and to highlight tab characters correctly,
;; this mode controls the display text property of some characters via font-lock.
;; Therefore, this mode may or may not play well with other modes that use the display text property.
;; This mode may also interfere with modes that use a display table to modify how whitespace is drawn,
;; e.g., the whitespace minor mode.
...
(evil-define-key 'normal package-menu-mode-map (kbd "C-S-u") 'package-menu-mark-upgrades)
